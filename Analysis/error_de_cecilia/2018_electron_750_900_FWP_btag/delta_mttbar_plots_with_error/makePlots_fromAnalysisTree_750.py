...
#stackList = {"TTToHadronic_2018":[kRed-3], "TTToOthers":[kRed+2], "TTToSemiLeptonic_2018":[kRed],"DYJetsToLL_M-50_HT_2018":[kBlue], "QCD_HT_2018":[kTeal], "WJetsToLNu_2018":[kGreen], "ST_2018":[kYellow], "WW_WZ_ZZ_2018v3":[kOrange]}
stackList = { "TTToSemiLeptonic_2018":[kRed], "Others":[kBlue], "WJetsToLNu_2018":[kGreen]}
...
sample_names = ["DYJetsToLL_M-50_HT_2018", "QCD_HT_2018", "ST_2018",  "WJetsToLNu_2018","TTToSemiLeptonic_2018"]
...
for sample in sample_names:
		...
		tree_MC[sample]=_file[sample].Get("AnalysisTree")
 	        ...
 	        tree_MC[sample].Draw("%s>>h2_%s(%i,%i,%f)"%(histName,sample,histograms[histName][2],histograms[histName][3][0],histograms[histName][3][1]),"(Mttbar > 750 && Mttbar < 900 && ttagN <= %s && wtagN <= %s && btagN >= 1 && rec_chi2<30)*weight*(weight_sfelec_TightID)*(weight_pu)*(weight_sfelec_Trigger)*(weight_toptagSF_)*(weight_pt_rew_nolimit)*(weight_btag)*(weight_sfelec_Rec)*0.85"%(argv[2],argv[3]))
                ...
                tree_MC_up[sample].Draw("%s>>h3_%s(%i,%i,%f)"%(histName,sample,histograms[histName][2],histograms[histName][3][0],histograms[histName][3][1]),"(Mttbar > 750 && Mttbar < 900 && ttagN <= %s && wtagN <= %s && btagN >= 1 && rec_chi2<30)*weight*(weight_sfelec_TightID)*(weight_pu)*(weight_sfelec_Trigger)*(weight_toptagSF_)*(weight_pt_rew_nolimit)*(weight_btag)*(weight_sfelec_Rec)*1.05"%(argv[2],argv[3]))
...
tree.Draw("%s>>dat_hist(%i,%i,%f)"%(histName,histograms[histName][2],histograms[histName][3][0],histograms[histName][3][1]),"Mttbar > 750 && Mttbar < 900 && ttagN <= %s && wtagN <= %s && btagN >= 1 && rec_chi2<30"%(argv[2],argv[3]))
...
if log:
	...
	canvasRatio.SaveAs("%s_new_750.pdf"%(histName))
