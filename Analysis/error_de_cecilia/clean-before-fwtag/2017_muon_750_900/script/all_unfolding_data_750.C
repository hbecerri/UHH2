...
void all_unfolding_data_750(string var_name = "", string var_gen = "", string region = "", string year = "2017")
{
...
//    string  jerDown = "(weight_sfmu_HighPtID)*(weight_pu)*(weight_sfmu_Trigger)*(weight_toptagSF_)*(weight_pt_rew_nolimit)*(weight_btagdisc_central)*(muonrecSF_nominal)*(weight_HT_HT)";
    ...
    string selcuts_boosted_central = Form("(ttagN <= 1 && btagN >= 1 && wtagN <= 1 && rec_chi2 < 30 && Mttbar > 750 && Mttbar < 900 && %s > 0 && %s < 900)*%f",Mttbar_gen.c_str(),Mttbar_gen.c_str(),mistag_nominal);
