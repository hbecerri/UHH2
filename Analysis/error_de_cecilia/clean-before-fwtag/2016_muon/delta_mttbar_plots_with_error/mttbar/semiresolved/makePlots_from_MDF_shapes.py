...
for sample in sample_names:
    _file[sample] = TFile("todo.root","read")
    ...
    if(legend_index == 0 and sample != "QCD" and sample!= "DY"):
        if(sample == "ST"):
...
_file["Data"] = TFile("todo.root","read")
...
for jj in range(1,errorban.GetNbinsX() + 1):
    errorban.SetBinError(jj, math.sqrt(errorban.GetBinError(jj)*errorban.GetBinError(jj)*100))
    temp.SetBinError(jj, math.sqrt(temp.GetBinError(jj)*temp.GetBinError(jj)*100))
...
if log:
    ...
    canvasRatio.SaveAs("semiresolved.pdf")
