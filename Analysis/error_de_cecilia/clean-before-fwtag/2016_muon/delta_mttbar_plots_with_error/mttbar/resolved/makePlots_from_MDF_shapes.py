...
for jj in range(1,errorban.GetNbinsX() + 1):
    errorban.SetBinError(jj, math.sqrt(errorban.GetBinError(jj)*errorban.GetBinError(jj)*200))
    temp.SetBinError(jj, math.sqrt(temp.GetBinError(jj)*temp.GetBinError(jj)*200))
...
if log:
    ...
    canvasRatio.SaveAs("resolved.pdf")
