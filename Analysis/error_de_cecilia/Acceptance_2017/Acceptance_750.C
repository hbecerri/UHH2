...
void Acceptance_750(string var_weight = "wgtMC__PDF[1]", int aa = 1)
{
...
//----obetener_toda_la_informacion_de_entrada--------??
    ...
    chreco_ttbar_antes_muon->Add("/nfs/dust/cms/user/hugobg/analysis/Preselection_Acceptance_2017/uhh2.AnalysisModuleRunner.MC.TTToSemiLeptonic_2017v2_24.root/AnalysisTree");
    ...
    chreco_ttbar_despues_electron->Add("/nfs/dust/cms/user/hugobg/ZPrime_102X/analysis_output/2017_CHS/electron/workdir_Zprime_Analysis_2017_electron_CHS_Acceptance/uhh2.AnalysisModuleRunner.MC.TTToSemiLeptonic_2017v2_0.root/AnalysisTree");
...
//array for variable 
    ...
    treereco_ttbar_despues_muon->Project("Acc_despues_muon",Form("%s < %f ? TMath::Max(%f,%s): (%s > %f ? TMath::Min(%f,%s) : %s)",var_gen.c_str(),bins_gen[0]+0.01,bins_gen[0]+0.01,var_gen.c_str(),var_gen.c_str(),bins_gen[binnum_gen]-0.01,bins_gen[binnum_gen]-0.01,var_gen.c_str(),var_gen.c_str()),Form("(ttagN <= 1 && btagN >= 1 && wtagN <= 1 && rec_chi2 < 30 && Mttbar > 750 && Mttbar < 900)*%s",var_weight.c_str())); 
    treereco_ttbar_despues_electron->Project("Acc_despues_electron",Form("%s < %f ? TMath::Max(%f,%s): (%s > %f ? TMath::Min(%f,%s) : %s)",var_gen.c_str(),bins_gen[0]+0.01,bins_gen[0]+0.01,var_gen.c_str(),var_gen.c_str(),bins_gen[binnum_gen]-0.01,bins_gen[binnum_gen]-0.01,var_gen.c_str(),var_gen.c_str()),Form("(ttagN <= 1 && btagN >= 1 && wtagN <= 1 && rec_chi2 < 30 && Mttbar > 750 && Mttbar<900)*%s",var_weight.c_str())); 


    treereco_ttbar_despues_muon->Project("Acc_despues_muon_nominal",Form("%s < %f ? TMath::Max(%f,%s): (%s > %f ? TMath::Min(%f,%s) : %s)",var_gen.c_str(),bins_gen[0]+0.01,bins_gen[0]+0.01,var_gen.c_str(),var_gen.c_str(),bins_gen[binnum_gen]-0.01,bins_gen[binnum_gen]-0.01,var_gen.c_str(),var_gen.c_str()),"ttagN <= 1 && btagN >= 1 && wtagN <= 1 && rec_chi2 < 30 && Mttbar > 750 && Mttbar < 900");
    treereco_ttbar_despues_electron->Project("Acc_despues_electron_nominal",Form("%s < %f ? TMath::Max(%f,%s): (%s > %f ? TMath::Min(%f,%s) : %s)",var_gen.c_str(),bins_gen[0]+0.01,bins_gen[0]+0.01,var_gen.c_str(),var_gen.c_str(),bins_gen[binnum_gen]-0.01,bins_gen[binnum_gen]-0.01,var_gen.c_str(),var_gen.c_str()),"ttagN <= 1 && btagN >= 1 && wtagN <= 1 && rec_chi2 < 30 && Mttbar > 750 && Mttbar < 900");
    ...
    Double_t variancia_muon_ttbar1 = Acceptance_muon_ttbar1/Acceptance_muon_ttbar1_nominal; 
    Double_t variancia_muon_ttbar2 = Acceptance_muon_ttbar2/Acceptance_muon_ttbar2_nominal;
    Double_t variancia_electron_ttbar1 = Acceptance_electron_ttbar1/Acceptance_electron_ttbar1_nominal;
    Double_t variancia_electron_ttbar2 = Acceptance_electron_ttbar2/Acceptance_electron_ttbar2_nominal;

    ofstream electron1;
    electron1.open ("electron_ttbar1_750.txt", ios::app);
    electron1 << aa;
    electron1 << "\t";
    ...
    ofstream electron2;
    electron2.open ("electron_ttbar2_750.txt", ios::app);
    electron2 << aa;
    electron2 << "\t";
    ...
    ofstream muon1;
    muon1.open ("muon_ttbar1_750.txt", ios::app);
    muon1 << aa;
    muon1 << "\t";
    ...
    ofstream muon2;
    muon2.open ("muon_ttbar2_750.txt", ios::app);
    muon2 << aa;
    muon2 << "\t";
    muon2 <<  variancia_muon_ttbar2;
    muon2 << "\n";

    cout << Acceptance_muon_ttbar1_nominal << endl;// = Acc_despues_muon->GetBinContent(1)/Acc_antes_muon->GetBinContent(1);
    cout << Acceptance_muon_ttbar2_nominal << endl; //= Acc_despues_muon->GetBinContent(2)/Acc_antes_muon->GetBinContent(2);
    cout << Acceptance_electron_ttbar1_nominal << endl; //= Acc_despues_electron->GetBinContent(1)/Acc_antes_electron->GetBinContent(1);
    cout << Acceptance_electron_ttbar2_nominal << endl;
