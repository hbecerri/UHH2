...
stackList = { "TTToOthers":[kRed+3], "TTToSemiLeptonic_2018":[kRed], "DYJetsToLL_M-50_HT_2018":[kBlue], "QCD_HT_2018":[kTeal], "WJetsToLNu_2018":[kGreen], "ST_2018":[kYellow], "WW_WZ_ZZ_2018":[kOrange]}
...
histograms = {
              ...
              "Mttbar": ["Mttbar", "Events", 15, [750, 900]], 
...
for sample in sample_names:
		...
		tree2_MC[sample]=_file[sample].Get("AnalysisTree")
	        tree2_MC[sample].Draw("%s>>h2_%s(%i,%i,%f)"%(histName,sample,histograms[histName][2],histograms[histName][3][0],histograms[histName][3][1]),"(ttagN <= 1 && wtagN <=1 && btagN >= 1 && Mttbar > 750 && Mttbar<900 && rec_chi2<30)*weight*(weight_sfelec_TightID)*(weight_pu)*(weight_sfelec_Trigger)*(weight_toptagSF_)*(weight_pt_rew_nolimit)*(weight_btagdisc_central)*(weight_sfelec_Rec)*(weight_HT_HT)*0.86")
...
if log:
	...
	canvasRatio.SaveAs("%s_750.pdf"%(histName))
