...
legend = TLegend(2*legendStart - legendEnd , 0.99 - (T/H)/(1.-padRatio+padOverlap) - 0.6*legendHeightPer/(1.-padRatio+padOverlap)*round((len(legList)+1)/2.), legendEnd, 0.99-(T/H)/(1.-padRatio+padOverlap))
legend.SetNColumns(3)


legendR = TLegend(2*legendStart - legendEnd , 0.99 - (T/H)/(1.-padRatio+padOverlap) - 0.65*legendHeightPer/(1.-padRatio+padOverlap)*round((len(legList)+1)/2.)-0.1, legendEnd, 0.99-(T/H)/(1.-padRatio+padOverlap))
...
sample_names = ["DYJetsToLL_M-50_HT_2017", "QCD_HT_2017", "WJetsToLNu_2017", "ST_2017", "TTToSemiLeptonic_2017","TTToOthers"]
...
for sample in sample_names:
		...
		tree_MC[sample]=_file[sample].Get("AnalysisTree")
 	        ...
 	        tree_MC[sample].Draw("%s>>h2_%s(%i,%i,%f)"%(histName,sample,histograms[histName][2],histograms[histName][3][0],histograms[histName][3][1]),"weight*weight_sfmu_HighPtID*weight_sfmu_Trigger*weight_pu*weight_toptagSF_*weight_pt_rew*weight_btagdisc_central*muonrecSF_nominal*0.95*(ttagN <= 1 && wtagN <= 1 && btagN>=1 && rec_chi2 < 30 && Mttbar > 900)")
		...
		legendR.AddEntry(hist1_[sample],sample,'f')       
        	hist1_[sample].SetYTitle(histograms[histName][1])        
                ...
                tree_MC_up[sample].Draw("%s>>h3_%s(%i,%i,%f)"%(histName,sample,histograms[histName][2],histograms[histName][3][0],histograms[histName][3][1]),"weight*weight_sfmu_HighPtID_up*weight_sfmu_Trigger_up*weight_pu_up*weight_toptagSF_up_*1.0*muonrecSF_up*(ttagN <= 1 && wtagN <= 1 && btagN>=1 && rec_chi2 < 30 && Mttbar > 900)")
