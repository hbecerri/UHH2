...
histograms = {
              ...
              "Ak4_j2_pt": ["Ak4_j2_pt", "Events", 22, [150,1200]],              
              "Ak4_j3_pt": ["Ak4_j3_pt", "Events", 22, [150,1200]],
...
sample_names = ["DYJetsToLL_M-50_HT_2018", "QCD_HT_2018", "WJetsToLNu_2018", "ST_2018", "TTToSemiLeptonic_2018","TTToOthers"]
...
for sample in sample_names:
		...
		tree_MC[sample]=_file[sample].Get("AnalysisTree")
	        ...
	        tree_MC[sample].Draw("%s>>h2_%s(%i,%i,%f)"%(histName,sample,histograms[histName][2],histograms[histName][3][0],histograms[histName][3][1]),"weight*weight_sfelec_TightID*weight_sfelec_Trigger*weight_pu*weight_toptagSF_*weight_pt_rew*weight_btagdisc_central*weight_sfelec_Rec*0.85*(ttagN <= 1 && wtagN <= 1 && btagN>=1 && rec_chi2 < 30 && Mttbar < 4000 && Ak4_j1_pt > 200 )")
		...
		legendR.AddEntry(hist1_[sample],sample,'f')       
        	hist1_[sample].SetYTitle(histograms[histName][1])        
                ...
                tree_MC_up[sample].Draw("%s>>h3_%s(%i,%i,%f)"%(histName,sample,histograms[histName][2],histograms[histName][3][0],histograms[histName][3][1]),"weight*weight_sfelec_TightID_up*weight_sfelec_Trigger_up*weight_pu_up*weight_toptagSF_up_*0.87*weight_sfelec_Rec*(ttagN <= 1 && wtagN <= 1 && btagN>=1 && rec_chi2 < 30 && Mttbar < 4000 && Ak4_j1_pt > 200)")
...
tree.Draw("%s>>dat_hist(%i,%i,%f)"%(histName,histograms[histName][2],histograms[histName][3][0],histograms[histName][3][1]),"(ttagN <= 1 && wtagN <= 1 && btagN>=1 && rec_chi2 < 30 && Mttbar < 4000 && Ak4_j1_pt > 200)")
